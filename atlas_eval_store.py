...
def _write_episode_eval_text_file(outputs_dir: Path, episode_id: str, text: str) -> Optional[Path]:
    ...
    # Keep eval text separate from timed labels to avoid polluting chat timeline inputs.
    out_path = target_dir / f"text_{eid}_chat_eval.txt"
...
def upsert_evaluation(
    ...
    try:
        _write_episode_eval_text_file(outputs_dir, eid, text)
    except Exception:
        pass
    return path
